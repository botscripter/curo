...
open class CamundaVariableHelper(private val variableScope: VariableScope) {
    ...
    operator fun <T : Any> get(variableDefinition: CamundaVariableDefinition<T>, defaultValue: T? = null): T {
        ...
        ?: throw CamundaVariableException("Camunda variable '" + variableDefinition.variableName + "' is not defined")
    }
    ...
    operator fun <T : Any> get(
        ...
        ?: throw CamundaVariableException("Camunda variable '" + variableListDefinition.variableName + "' is not defined")
    }
    ...
    fun <T : Any> getOrNull(variableDefinition: CamundaVariableDefinition<T>): T? {
        return try {
            val raw = variableScope.getVariableTyped<TypedValue>(variableDefinition.variableName, true)
            if (raw != null) {
                when (raw.type) {
            ...
            throw CamundaVariableException(
                "Could not cast variable '" + variableDefinition.variableName + "' to type " + variableDefinition.type.toString(),
                e
            )
        }
    }
    ...
    fun <T : Any> getOrNull(variableListDefinition: CamundaVariableListDefinition<T>): List<T?>? {
        return try {
            val raw = variableScope.getVariableTyped<TypedValue>(variableListDefinition.variableName, true)
            if (raw != null) {
            ...
            throw CamundaVariableException(
                "Could not cast variable '" + variableListDefinition.variableName + "' to type " + variableListDefinition.type.toString(),
                e
            )
        }
    }
    ...
    operator fun <T : Any> set(variableName: CamundaVariableDefinition<T>, value: T?) {
        variableScope.setVariable(variableName.variableName, value)
    }
    ...
    fun initVariables(variableClass: Any, convertExistingVariables: Boolean = true) {
        ...
        variableClass.javaClass.declaredFields
            ...
            .forEach {
                ...
                if (convertExistingVariables && variableScope.hasVariable(definition.variableName)) {
                    val raw = variableScope.getVariableTyped<TypedValue>(definition.variableName, true)
                    if (raw.type.isPrimitiveValueType) {
                        ...
                        try {
                            ...
                            variableScope.setVariable(definition.variableName, castedVariable)
                            logger.debug("CURO: \t-> Changed variable '${definition.variableName}' to defined type '${definition.type}'")
                        } catch (e: Exception) {
                            logger.debug("CURO: \t-> Changed variable '${definition.variableName}' to defined type '${definition.type}' failed!! -> Variable got skipped")
                        }
                    }
                }
                ...
                if (it.isAnnotationPresent(InitWithNull::class.java) || it.isAnnotationPresent(InitWithEmpty::class.java) || doAllEmpty || doAllNull) {
                    ...
                    when {
                        (BeanUtils.isSimpleValueType((it.genericType as ParameterizedType).actualTypeArguments[0] as Class<*>)) && emptyInit -> {
                            ...
                            variableScope.setVariable(definition.variableName, value)
                            logger.debug("CURO: \t-> Initiate variable '${definition.variableName}' with empty value")
                        }
                        (BeanUtils.isSimpleValueType((it.genericType as ParameterizedType).actualTypeArguments[0] as Class<*>)) && !emptyInit -> {
                            ...
                            logger.debug("CURO: \t-> Initiate variable '${definition.variableName}' with null value")
                        }
